...
export interface DashboardData {
  ...
  /* Ano anterior mês a mês — usado para calcular crescimento real */
  anoAnterior: { mes: string; valor: number }[];
}
...
export function parseExcelFile(buffer: ArrayBuffer): DashboardData {
  ...
  /* Nova aba: "Ano Anterior"
     Estrutura esperada:
     Linha 1: cabeçalho (Mês | Faturamento)
     Linhas 2-13: Jan | 100000  /  Fev | 120000 ... */
  const aa = readSheet(wb, 'Ano Anterior');
  ...
  return {
    ...
    anoAnterior: aa.slice(1).map((r) => ({ mes: r[0] ?? '', valor: r[1] ?? 0 })),
  };
}
